...
def on_server_message0(word, word_eol, userdata):
	print(f"{GREEN}wordLen:{ENDC}", len(word))
	print(f"{GREEN}word:{ENDC}", word)
	print(f"{GREEN}word_eol:{ENDC}", word_eol)
	message = word_eol[0]
	if "001":
	...
	# else:
	# 	hexchat.prnt(f"{LIGHT_GREY}{message}{ENDC}")
	return hexchat.EAT_NONE


def on_server_message(word, word_eol, userdata):
	if word_eol:
		hexchat.prnt(f"{LIGHT_GREY}------------- HANDSHAKE WITH SERVER ------------- {len(word_eol)}{ENDC}")
	# hexchat.prnt("Welcome message received from server: {}".format(word_eol[0]))
	# print(f"{GREEN}wordLen:{ENDC}", len(word))
	# print(f"{GREEN}word:{ENDC}", word)
	# print(f"{GREEN}word_eol:{ENDC}", word_eol)
...
#--------------------------------------------------------------------------------
# 
def on_server_messagex(word, word_eol, userdata):
	if word_eol:
		message = word_eol[0]
		char_count = len(message)
		hexchat.prnt("Server text received: {} ({} characters)".format(message, char_count))
	return hexchat.EAT_NONE
...
# def on_server_text(word, word_eol, userdata):
	...
	# 	message = word_eol[0]
	# 	char_count = len(message)
	# 	hexchat.prnt(f"{LIGHT_GREY}Server text received: {message} ({char_count} characters)")
	# return hexchat.EAT_NONE
	# return hexchat.EAT_PLUGIN
	# return hexchat.EAT_HEXCHAT


# hexchat.hook_server("RAW LINE", on_server_messagex)
# hexchat.hook_print("Server Text", on_server_text)
hexchat.hook_server("001", on_server_messagex)
hexchat.prnt("Server Message Hook script loaded")
